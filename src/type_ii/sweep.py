"""One-variable sweep loop for the standalone Type-II method."""
...
_MODEL_VARIABLES = {"N1", "K", "n_pilots", "L"}
...
def _set_sweep_value(base, training, variable: str, value):
    ...
    raise ValueError(
        f"unknown sweep_variable={variable!r}; choose one of {allowed}")


def _summary_row(result: dict, sweep_variable: str, value, output_dir: str):
    final = result["final"]
    ...
    return {
        ...
        "N1": result["model_config"]["N1"],
        "K": result["model_config"]["K"],
        "n_pilots": result["model_config"]["n_pilots"],
        "Lp": result["train_config"]["Lp"],
        "snr_db": result["train_config"]["snr_db"],
        "L": result["model_config"]["L"],
        "amp_bits": result["model_config"]["amp_bits"],
        "phase_bits": result["model_config"]["phase_bits"],
        "feedback_bits": result["model_config"]["feedback_bits"],
        "fsq_feature_dim": len(result["model_config"]["fsq_levels"]),
        "fsq_target_bits_per_feature": result["model_config"].get(
            "fsq_target_bits_per_feature"),
        "fsq_levels": json.dumps(result["model_config"]["fsq_levels"]),
        "fsq_bits_used": fsq_bits(result["model_config"]["fsq_levels"]),
        "initialization_mode": (
        ...
        "final_continuous_rate": final.get("continuous_rate"),
...
def run_sweep(
    ...
    *,
    ...
    sweep_root, method, summary_dir = prepare_layout(
        output_root, sweep_variable, "type_ii", training.name)
    rows = []
    completed = {}
    for value in values:
        ...
        output_dir = run_directory(
            sweep_root, sweep_variable, _label(value),
            "type_ii", method, train_cfg.seed,
        )
        ...
        if result is None:
            ...
            print("#" * 78)
            print(
                f"# {training.name} | {sweep_variable}={value} | "
                f"seed={train_cfg.seed} | "
                f"init={run_init_label}")
            print(
                f"# feedback: B={model_cfg.feedback_bits} | "
        ...
        if reuse_info.get("transfer_kind") == "anchor_reuse_no_training":
            # No best.pt exists here; later chain links load the anchor's.
